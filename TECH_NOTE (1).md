...
*[Member 5 — `results/scores.md` still needs the actual chatbot-vs-answer-key
scoring pass filled in now that the answer key is complete.]*

## Boundary Testing / Hallucination Case

**Case: unsupported inference on AI/cloud capex (Q10)**
...
The chatbot's arithmetic was correct — Microsoft ~22.9% of revenue, Alphabet
~22.7%, Amazon ~18.4% — but it then concluded Microsoft was "investing most
aggressively in AI/cloud infrastructure." The 10-Ks only disclose **company-wide**
capex; they don't break out AI/cloud-specific spending. The model presented an
inference as if it were a fact directly stated in the filings.

- **Failure type:** unsupported inference (not a factual/arithmetic error)
- **Hallucination:** yes — scored 2/3
- **Corrected answer:** state the company-wide capex-intensity ranking as-is,
  with an explicit caveat that it isn't a precise AI/cloud-only comparison

**Why this case matters:** it shows that correct retrieval and correct math
don't guarantee a reliable conclusion — a model can still overreach in how it
interprets numbers it retrieved accurately. This is a subtler and more
instructive failure than the fabricated-numbers hallucinations logged
elsewhere (see Model Choice section above).

**How to reduce it:**
1. Require the model to distinguish directly-reported facts from calculated
   values from inferred conclusions
2. Add a prompt rule requiring a caveat whenever the requested metric isn't
   separately disclosed
3. Ask the model to state whether each conclusion is directly supported by
   the filing text
4. Retrieve nearby explanatory notes rather than relying only on the raw
